	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<div class="alert alert-warning"><strong><center>BUAT JADWAL PEMASANGAN</center></strong></div>
			</div>
			<div class="modal-body">
					...
					<div class="col-sx-10">
						<select class = "form-control" name="teknisi" required="required">
				            ...
				            foreach($select->select_data_teknisi() as $data) {
				            	$selected="";
				            	if($id_teknisi==$data['id_user']){
				            		$selected="selected='selected'";
				            	}
				            ?>
							<option value="<?php echo $data['id_user'];?>" <?php echo $selected;?>><?php echo $data['nama_user'];?></option>
							<?php
							}
							?>
						</select>
				...
				<div class = "modal-footer">
					...
					<button name = "tombol_buat_jadwal_pemasangan" class="btn btn-warning">JADWALKAN</button>
				</div>
				</form> 
			</div>                                
		</div>
