...
<section class="content-header">
  <h1 class="">
    ...
    &nbsp;
    <a href="pdf_laba_rugi_overhead.php?tanggal_awal=<?php echo $tanggal_awal;?>&tanggal_akhir=<?php echo $tanggal_akhir;?>" target="_blank" class="btn btn-primary">Download</a>
  </h1>
</section>
    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <center>
            <h2>
              <strong>PT. SOLO JALA BUANA</strong>
            </h2>
            <h3>
          ...
          <table class="table table-condensed">
            ...
            foreach ($select->select_jenis_account_where_id_master_jenis_account('4') as $data_jenis_account) {
              ...
              foreach ($select->select_account_where_id_jenis_account($id_jenis_account) as $data_account) {
                ...
                $nominal = $select->select_jumlah_nominal_per_account_kredit_overhead($id_account, $tanggal_awal, $tanggal_akhir);
            ?>
            <tr>
              <td width="50">&nbsp;</td>
            ...
            foreach ($select->select_jenis_account_where_id_master_jenis_account('5') as $data_jenis_account) {
              ...
              foreach ($select->select_account_where_id_jenis_account($id_jenis_account) as $data_account) {
                $id_account = $data_account['id_account'];
                $nominal = $select->select_jumlah_nominal_per_account_debit_overhead($id_account, $tanggal_awal, $tanggal_akhir);
            ?>
            <tr>
              <td width="50">&nbsp;</td>
              <td><?php echo $data_account['nama_account'];?></td>
