...
class crud {
	...
	function insert_pembayaran_access_bulanan() {
		...
		if($_POST['penagih']=="eko"){
			    $keterangan_data_temp = $keterangan_data_temp." | Pak Eko";
        }
        
        $keterangan_data_temp = $keterangan_data_temp.$tambahan_keterangan;
        
        $keterangan_data_temp_restitusi = "Restitusi Internet ( ".$nama_user." ( ".$catatan." ) )";

		//proses data temp
		if($bayar>=1){
		    mysqli_query($koneksi, "INSERT INTO `data_temp`(`tanggal`, `keterangan`, `no_log_pembayaran`, `nominal`, `id_account`, `in_out`, `status`, `ekstra`) VALUES ('$waktu_pembayaran', '$keterangan_data_temp', '$no_log_pembayaran', '$bayar', '$id_account', '$in_out', '$status', '$ekstra')") or die(mysqli_error());
		}
		
		if($restitusi>=1){
		    mysqli_query($koneksi, "INSERT INTO `data_temp`(`tanggal`, `keterangan`, `no_log_pembayaran`, `nominal`, `id_account`, `in_out`, `status`, `ekstra`) VALUES ('$waktu_pembayaran', '$keterangan_data_temp_restitusi', '$no_log_pembayaran', '$restitusi', '$id_account', 'o', '$status', '$ekstra')") or die(mysqli_error());
		}
	...
	function insert_pembayaran_access_user_baru() {
		...
		$nominal_po = $tampil_register['biaya_registrasi'] + $tampil_register['monthly_fee'];

		$hak_akses = $nominal_po;
		...
		if($total_dana >= $hak_akses) {
			...
			if($total_dana>=$total_po) {
				...
				//melihat apakah uang sisanya lebih dari harga pake, kerena uang itu banti akan langsung di input ke tagihan bulanan selanjutnya
				if($update_sisa_pembayaran<$harga_paket){
					//jika sisanya kurang dari harga paket, maka sisanya langsung di masukan ke dalam billing_saldo
					//data yang dirubah : billing_saldo, billing_total_bayar, billing_restitusi, status, tanggal_close
					mysqli_query($koneksi,"update sale_register set billing_saldo='$update_sisa_pembayaran', billing_total_bayar=billing_total_bayar+'$bayar', billing_total_restitusi=billing_total_restitusi+'$restitusi', status='Close', tanggal_close='$tanggal_close' where id_register='$id_register'");
				
				    $bulan=date('Y-m-d');

	                mysqli_query($koneksi, "insert into billing_backup_log_user (id_register, id_paket, bulan) values ('$id_register', '$id_paket', '$bulan')");
				} else {
					//jika sisanya lebih dari harga paket, maka di cek dulu apkah dia punya tunggakan
					if($bulan_berjalan>$bulan_terbayar){
						//jika dia punya tunggakan maka kita cek banyakan nominal tunggakan atau uang sisa
						$tunggakan = $bulan_berjalan-$bulan_terbayar;
						$nominal_tunggakan = $tunggakan*$harga_paket;

						//kita cek, apakah uang sisanya lebih besar dari nominal tunggakan
						if($update_sisa_pembayaran>=$nominal_tunggakan){
							//jika iya tinggal kita proses uangnya lebih banya, tinggal kita kurangi sisa sama tunggakan
							$update_sisa_pembayaran = $update_sisa_pembayaran - $nominal_tunggakan;
							mysqli_query($koneksi,"update sale_register set billing_saldo='$update_sisa_pembayaran', billing_total_bayar=billing_total_bayar+'$bayar', billing_total_restitusi=billing_total_restitusi+'$restitusi', status='Close', tanggal_close='$tanggal_close', billing_bulan_terbayar=billing_bulan_berjalan where id_register='$id_register'");
						
						    $bulan=date('Y-m-d');

				            mysqli_query($koneksi, "insert into billing_backup_log_user (id_register, id_paket, bulan) values ('$id_register', '$id_paket', '$bulan')");
						} else {
							//jika uang tidak lebih dari sama dengan nominal tunggakan maka kita cari seberapa bisa nominal itu untuk mengurangi tunggakan
							$sisa_uang_mod_harga_paket = $update_sisa_pembayaran%$harga_paket;
							$update_sisa_pembayaran = $update_sisa_pembayaran-$sisa_uang_mod_harga_paket;
							$jumlah_tunggakan_yang_bisa_dikurangi = $update_sisa_pembayaran/$harga_paket;

							mysqli_query($koneksi,"update sale_register set billing_saldo='$sisa_uang_mod_harga_paket', billing_total_bayar=billing_total_bayar+'$bayar', billing_total_restitusi=billing_total_restitusi+'$restitusi', status='Close', tanggal_close='$tanggal_close', billing_bulan_terbayar=billing_bulan_terbayar+'$jumlah_tunggakan_yang_bisa_dikurangi' where id_register='$id_register'");
						...
						//jika tidak punya tunggakan maka langsung saja uang sisanya di masukan kedalam billing_saldo
						mysqli_query($koneksi,"update sale_register set billing_saldo='$update_sisa_pembayaran', billing_total_bayar=billing_total_bayar+'$bayar', billing_total_restitusi=billing_total_restitusi+'$restitusi', status='Close', tanggal_close='$tanggal_close' where id_register='$id_register'");
					}
				}
			...
			$keterangan_data_temp = "Pembayaran User Baru ( ".$nama_user." )";
			$keterangan_data_temp_2 = "Pembayaran Alat ( ".$nama_user." )".$tambahan_keterangan;
			$id_account = $via;
			$in_out = "i";
			$status = "belum";
			$ekstra = "access";
			...
			$nominal_alat = $total_dana - $hak_akses;

			//proses data temp
			$query_data_temp = mysqli_query($koneksi,"SELECT * FROM `data_temp` WHERE `keterangan` = '$keterangan_data_temp'");
			$jumlah_data_temp = mysqli_num_rows($query_data_temp);
			if($jumlah_data_temp <= 0){
    			mysqli_query($koneksi, "INSERT INTO `data_temp`(`tanggal`, `keterangan`, `no_log_pembayaran`, `nominal`, `id_account`, `in_out`, `status`, `ekstra`) VALUES ('$waktu_pembayaran', '$keterangan_data_temp', '$no_log_pembayaran', '$hak_akses', '$id_account', '$in_out', '$status', '$ekstra')") or die(mysqli_error());
    
    			mysqli_query($koneksi, "INSERT INTO `data_temp`(`tanggal`, `keterangan`, `no_log_pembayaran`, `nominal`, `id_account`, `in_out`, `status`, `ekstra`) VALUES ('$waktu_pembayaran', '$keterangan_data_temp_2', '', '$nominal_alat', '$id_account', '$in_out', '$status', '$ekstra')") or die(mysqli_error());
			}
			
			if($restitusi>=1){
			    $keterangan_data_temp_restitusi = "Restitusi User Baru ( ".$nama_user." ( ".$catatan." ) )";
			    
		        mysqli_query($koneksi, "INSERT INTO `data_temp`(`tanggal`, `keterangan`, `no_log_pembayaran`, `nominal`, `id_account`, `in_out`, `status`, `ekstra`) VALUES ('$waktu_pembayaran', '$keterangan_data_temp_restitusi', '$no_log_pembayaran', '$restitusi', '$id_account', 'o', '$status', '$ekstra')") or die(mysqli_error());
		    }

		} else {
			echo '<script>alert("Maaf, Nominal bayar harus di atas PRORATA dan REGISTRASI")</script>';
		}
	}
...
class select {
  ...
  function select_data_temp_where($via, $dk, $tanggal_awal, $tanggal_akhir, $isi_data) {    
    ...
    $query=mysqli_query($koneksi,"SELECT * FROM data_temp WHERE `keterangan` like '%$isi_data%' AND id_account like '%$via%' AND in_out like '%$dk%' AND tanggal BETWEEN '$tanggal_awal 00:00:00' AND '$tanggal_akhir 23:59:59' AND ekstra = 'access' ORDER BY `tanggal` DESC");
    while($tampil=mysqli_fetch_array($query))
    $data[]=$tampil;
    return $data;
  }
  ...
  function saldo_kas($tanggal_hari_ini) {
      ...
      $query_temp_in_kas = mysqli_query($koneksi,"SELECT SUM(`nominal`) as total FROM `data_temp` WHERE `tanggal` BETWEEN '0000-00-01 00:00:00' AND '$tanggal_hari_ini 23:59:59' AND `id_account`='1' AND `in_out`='i'");
      $data_temp_in_kas = mysqli_fetch_array($query_temp_in_kas);
      
      $query_temp_out_kas = mysqli_query($koneksi,"SELECT SUM(`nominal`) as total FROM `data_temp` WHERE `tanggal` BETWEEN '0000-00-01 00:00:00' AND '$tanggal_hari_ini 23:59:59' AND `id_account`='1' AND `in_out`='o' AND `keterangan` not like 'Restitusi%'");
